...
/* TB371FC_BLACKBOX (p39 original, p43 cache-clean, p45c restored): periodic +
 * panic copy of the kernel log tail into the no-map reserved region at
 * 0x27E000000, recoverable by a later APatch boot via physical-memory KPM. */
...
{
	return; /* V27M: blackbox disabled (suspected self-corruption) */
...
/* TB371FC_BB25: fixmap-based early markers. Mapping via __set_fixmap depends
 * ONLY on early_fixmap_init() (unlike early_memremap, which needs the early
 * ioremap bookkeeping). Markers dcache-cleaned -> survive warm reset. */
...
{
	if (!tb_mark_va) {
		__set_fixmap(FIX_TEXT_POKE0, TB_BB_PHYS, PAGE_KERNEL);
		tb_mark_va = (char *)__fix_to_virt(FIX_TEXT_POKE0);
		tb_mark_va[0x30] = 0x56;
		tb_mark_va[0x31] = 0x32;
		tb_mark_va[0x32] = 0x35;
	}
	if (tb_bb_seq < 0x3F) {
