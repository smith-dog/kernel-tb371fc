#!/bin/bash
# p209-build-n93.sh — v27n93 = n91 + p206 pen b3 00 gate-open (FACT-026 mandatory:
...
LOG=$BASE/logs/p209-build.log
exec > "$LOG" 2>&1
echo "=== P209 BUILD START $(date) ==="
...
python3 tb371fc/scripts/p208-n93-sde-gate.py || { echo P208_FAILED; exit 1; }
echo "--- patch state ---"
...
cp arch/arm64/boot/Image $BASE/out/Image-v27n93
...
python3 tools/repack_boot.py firmware/apatch_patched_11266_0.13.5_clte.img     out/Image-v27n93 out/boot-v27n93-pure-q706.img out/dtbs/tail-new.bin 2>&1 | tail -2
python3 tools/repack_boot.py out/kernelsu_patched_20260924_084550.img     out/Image-v27n93 out/boot-v27n93-kspatched-flash.img out/dtbs/tail-new.bin 2>&1 | tail -2
echo "=== banner ==="
strings out/Image-v27n93 | grep -m1 "Linux version"
echo "=== md5 ==="
md5sum out/Image-v27n93 out/boot-v27n93-pure-q706.img out/boot-v27n93-kspatched-flash.img
echo "=== P209 BUILD DONE $(date) ==="
