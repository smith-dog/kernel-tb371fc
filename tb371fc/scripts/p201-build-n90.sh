#!/bin/bash
# p201-build-n90.sh — v27n90 = n89 + p200 (TASK-033 real-144 stock-channel port).
...
LOG=$BASE/logs/p201-build.log
exec > "$LOG" 2>&1
echo "=== P201 BUILD START $(date) ==="
...
cp arch/arm64/boot/Image $BASE/out/Image-v27n90
...
python3 tools/repack_boot.py firmware/apatch_patched_11266_0.13.5_clte.img \
    out/Image-v27n90 out/boot-v27n90-pure-q706.img out/dtbs/tail-new.bin 2>&1 | tail -2
python3 tools/repack_boot.py out/kernelsu_patched_20260924_084550.img \
    out/Image-v27n90 out/boot-v27n90-kspatched-flash.img out/dtbs/tail-new.bin 2>&1 | tail -2
echo "=== banner ==="
strings out/Image-v27n90 | grep -m1 "Linux version"
echo "=== md5 ==="
md5sum out/Image-v27n90 out/boot-v27n90-pure-q706.img out/boot-v27n90-kspatched-flash.img
echo "=== P201 BUILD DONE $(date) ==="
