#!/bin/bash
# p51 — v27b 点火: v27 内核 + insecure ramdisk (ro.secure=0/ro.adb.secure=0)
...
SERIAL=HA1ZXE2M
LOG=logs/v27b-ignition-timeline.txt
: > $LOG
ts() { echo "[$(date +%H:%M:%S)] $*" | tee -a $LOG; }

ts "P51 v27b ignition start"
[ -f out/boot-v27b-q706.img ] || { ts FATAL_NO_IMAGE; exit 1; }
...
$FB flash boot out/boot-v27b-q706.img 2>&1 | tail -1 | tee -a $LOG
...
grab() {  # 一次快照, 追加到各文件
  ...
  } >> logs/v27b-evidence.txt 2>&1
}

RESULT=UNKNOWN; LAST_AD="(init)"; LAST_FB=0; NGRAB=0
: > logs/v27b-evidence.txt
...
if $FB devices 2>/dev/null | grep -q fastboot; then
  ...
  ts "WARN: no fastboot — device may still run v27b"
fi
...
ts "P51 DONE result=$RESULT"
echo "$RESULT" > logs/v27b-ignition-result.txt
