...
static char *fw_name = "tfa98xx_QS.cnt"; /* p247: TB371FC vendor container */

/* p249: sync-preload the container from /vendor/firmware into the kernel
 * firmware cache, so the later request_firmware() (which fires long after
 * the ueventd fallback window on this device) is served from cache. */
static int __init p249_preload_container(void)
{
	const struct firmware *fw = NULL;
	int rc;

	rc = request_firmware(&fw, fw_name, NULL);
	if (rc == 0 && fw) {
		pr_info("p249: preloaded %s (%zu bytes) into fw cache\n",
			fw_name, fw->size);
		release_firmware(fw);
		/* request_firmware caches until release; re-request later
		 * hits the cache. To persist across the early probe we rely
		 * on the cache holding the entry until the consumer loads. */
	} else {
		pr_info("p249: preload rc=%d (vendor fs not mounted yet)\n", rc);
	}
	return 0;
}
late_initcall_sync(p249_preload_container);
				...
				int num_dai)
{
	 ...
	 * SND_SOC_DAPM_AIF_IN("AIF Playback", "AIF Playback", 0, SND_SOC_NOPM, 0, 0),
	 * into:
	...
	if (widgets && num_widgets > 0)
		for(i = 0; i < num_widgets; i++) {
			...
			if((widgets[i].id == snd_soc_dapm_aif_in)
				...
				snprintf(buf, 50, "%s-%x" /* p251 */, widgets[i].sname,
					addr);
...
static struct snd_soc_dapm_widget tfa98xx_dapm_widgets_common[] = {
	/* Stream widgets */
	SND_SOC_DAPM_AIF_IN("AIF IN", "AIF Playback", 0, SND_SOC_NOPM, 0, 0), /* p251 */
	SND_SOC_DAPM_AIF_OUT("AIF OUT", "AIF Capture", 0, SND_SOC_NOPM, 0, 0), /* p251 */
...
static int tfa98xx_load_container(struct tfa98xx *tfa98xx)
{
	...
	mutex_lock(&tfa98xx_mutex);
	/* p253: container already loaded globally? attach this device without
	 * re-requesting the file: on TB371FC only the first amp's request ever
	 * completed, late probes failed fast and left tfa->cnt NULL ("invalid
	 * pointer to container file" at stream start, silent playback). The
	 * dummy descriptor drives the callback's already-loaded branch, which
	 * runs the full per-device init without reading cont->size/data. */
	if (tfa98xx_container != NULL) {
		const struct firmware p253_fw = { .size = 0, .data = NULL };
		tfa98xx_container_loaded(&p253_fw, tfa98xx);
		mutex_unlock(&tfa98xx_mutex);
		return 0;
	}
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	tfa98xx->codec = codec;
#endif
	(void)tfa98xx_load_container(tfa98xx); /* p242c: async via dsp_init work */
			     ...
			     const struct i2c_device_id *id)
{
	...
	if ((no_start == 0) && (no_reset == 0)) {
		...
		case 0x96:
			...
			break;
		default:
			pr_info("p236: unknown rev 0x%x, treating as TFA9894\n", reg & 0xff);
			tfa98xx->flags |= TFA98XX_FLAG_MULTI_MIC_INPUTS;
			tfa98xx->flags |= TFA98XX_FLAG_TDM_DEVICE;
			tfa98xx->flags |= TFA98XX_FLAG_SKIP_INTERRUPTS;
			break;
	 ...
	 */
	/* p240: match the Lenovo machine dai_link codec_name (tfa98xx.0034) */
	dev_set_name(&i2c->dev, "tfa98xx.%04x", i2c->addr);
	...
	pr_info("p245: dai[0].name=%s\n", dai[0].name);

#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
	ret = devm_snd_soc_register_component(&i2c->dev,
				&soc_codec_dev_tfa98xx, dai,
				ARRAY_SIZE(tfa98xx_dai));
	pr_info("p245: register_component rc=%d\n", ret);
#else
...
static struct of_device_id tfa98xx_dt_match[] = {
	{ .compatible = "tfa,tfa98xx" }, /* p232: TB371FC DT binding */
